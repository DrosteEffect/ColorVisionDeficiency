...
`brettel1997`, `vienot1999`, `cvdsim` and `daltonizer` work with both `Nx3` colormaps and `RxCx3` images as their algorithms depend solely on color. `machado2010` and `milic2015` work with `RxCx3` images only, because their algorithms also depend on spatial image structure.
...
Simulate a MATLAB colormap with the Viénot single-plane method for protanopia and deuteranopia:
